...
- `index.html`
  - Switched the main sovereign heritage project link from `AUX.IO` to `ARCHAI`.
...
- `archai.html`
  - Removed the embedded AUX.IO live-demo section so the ARCHAI page is no longer duplicating the standalone AUX.IO page.
  - Added a cleaner AUX.IO CTA panel under the ARCHAI project explanation.
...
- Local `http://localhost:8011/archai.html` is focused on ARCHAI and links out to AUX.IO rather than embedding it.
